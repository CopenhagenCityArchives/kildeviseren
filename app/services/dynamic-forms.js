...
angular.module('dynform', [])
  .directive('dynamicForm', ['$q', '$parse', '$http', '$templateCache', '$compile', '$document', '$timeout', function ($q, $parse, $http, $templateCache, $compile, $document, $timeout) {
    var supported = {
        ...
        //TEST
        'autocomplete': {element: 'autocomplete', editable: false, textBased: false}//, click_activation:true,data:'movies',on_type:'doSomething',"class":'ng-isolate-scope ng-prisinte ng-valid',ng_model:'data'}
      };
            ...
            var buildFields = function (field, id) {
              if (!angular.isDefined(supported[field.type]) || supported[field.type] === false) {
                ...
                if (field.type === 'number' || field.type === 'range') {
                  ...
                  if (angular.isDefined(field.data)) {newElement.attr('data', field.data);}
                  if (angular.isDefined(field.model)) {newElement.attr('ng-model', field.model);}
                  if (angular.isDefined(field.click_activation)) {newElement.attr('click-activation', field.click_activation);}
                  if (angular.isDefined(field.on_type)) {newElement.attr('on-type', field.on_type);}
                  if (angular.isDefined(field['class'])) {newElement.attr('class', field['class']);}
                  if (angular.isDefined(field['attr-placeholder'])) {newElement.attr('attr-placeholder', field['attr-placeholder']);}
                }
